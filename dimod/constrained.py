...
class ConstrainedQuadraticModel:
    ...
    def to_lp_file(self, *, spool_size: int = int(1e9)) -> tempfile.SpooledTemporaryFile:
        ...
        def sign(bias: float) -> str:
            return '-' if bias < 0 else '+'

        def abs_(bias: float) -> str:
        ...
        def label_error(label: Hashable, rule: str) -> str:
        ...
        def validate_label(label: Hashable):
            if not isinstance(label, str):
                label_error(label, 'labels must be strings')

            if not len(label):
                label_error(label, 'labels must be nonempty')

            if len(label) > 255:
                label_error(label, 'labels must be <= 255 characters in length')

            if not all(c in LABEL_VALID_CHARS for c in label):
                label_error(label, f'labels must be composed of only these characters: '
                                   f'{"".join(sorted(LABEL_VALID_CHARS))}')

            if any(label.startswith(c) for c in LABEL_INVALID_FIRST_CHARS):
                label_error(label, f'labels must not start with any of these characters: '
                                   f'{"".join(sorted(LABEL_INVALID_FIRST_CHARS))}')

        class ToLPFile(SpooledTemporaryFile):
            TARGET_LINE_LEN = 80

            def __init__(self):
                """Subclass :class:`SpooledTemporaryFile` to provide a custom `write` method that
                ...
                """
                super().__init__(max_size=spool_size, mode='wb')

                # length of current line, i.e. number of bytes written since last newline
                self._line_len = 0

            def write(self, bytes_: ByteString) -> None:
                """Check if writing ``bytes_`` would result in our line length target being
                exceeded, and if so, emit a newline.  Then write ``bytes_``.
                ...
                code below, must pass ``bytes_`` arguments that are sufficiently short, to avoid
                ...
                if rpos is not None:
                    self._line_len = len(bytes_) - 1 - rpos
                else:
                    self._line_len += len(bytes_)
        ...
        for var, bias in self.objective.linear.items():
            if bias:
                if not f.tell():
                    ...
                    f.write(b"Minimize\n")
                    f.write(b' obj: ')
                f.write(f"{sign(bias)} {abs_(bias)} {var} ".encode('utf8'))
        ...
        if self.objective.quadratic:
            ...
            for (u, v), bias in self.objective.quadratic.items():
                ...
                f.write(f"{sign(bias)} {abs_(2 * bias)} {u} * {v} ".encode('utf8'))
            f.write(b']/2 ')

        if self.objective.offset:
            ...
            if not f.tell():
                ...
                f.write(b"Minimize\n")
                f.write(b' obj: ')
            f.write(f"{sign(offset)} {abs_(offset)} ".encode('utf8'))

        # write the constraints
        f.write(b"\n\n")
        f.write(b"Subject To \n")

        for label, constraint in self.constraints.items():
            f.write(f' {label}: '.encode('utf8'))
            ...
            if constraint.lhs.quadratic:
                ...
                for (u, v), bias in constraint.lhs.quadratic.items():
                    f.write(f"{sign(bias)} {abs_(bias)} {u} * {v} ".encode('utf8'))
                f.write(b'] ')
