...
class AdjArrayBQM {
  public:
     ...
     */
    template <class B2>
    AdjArrayBQM(const B2 dense[], size_type num_variables, bool ignore_diagonal = false) {
    ...
    bias_type get_linear(variable_type v) const { return invars[v].second; }
    ...
    size_type degree(variable_type v) const {
        ...
        if ((unsigned)v == invars.size() - 1) return outvars.size() - invars[v].first;
