...
class ConstrainedQuadraticModel {
 public:
    ...
    // index_type add_constraint(abc::QuadraticModelBase<bias_type, index_type> model, Sense sense,
    // bias_type rhs);
...
void ConstrainedQuadraticModel<bias_type, index_type>::set_objective(
    ...
    // Should we accept an iterator instead and deprecate this one?
    assert(mapping.size() == objective.num_variables());
...
// template <class bias_type, class index_type>
// void ConstrainedQuadraticModel<bias_type, index_type>::set_objective(QuadraticModelBase&& other)
// {
//     std::move(objective, other);
// }
