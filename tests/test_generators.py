...
class TestSatisfiability(unittest.TestCase):
    ...
    def test_random_graph_ensembles(self):
        num_variables = 10
        for k in [3,4]:
            if k==4:
                num_clauses = round(0.64*num_variables)
            else:
                num_clauses = round(1.1*num_variables)
            num_stubs = num_clauses*k
            degrees = dimod.generators.satisfiability._cut_poissonian_degree_distribution(num_variables, num_stubs, cut=2)
            degrees = np.array(degrees,int)
            self.assertTrue(np.all(degrees >= 2))
            self.assertEqual(np.sum(degrees),num_stubs)

            interactions = dimod.generators.satisfiability._kmcsat_interactions(num_variables, k, num_clauses,
                                                                                graph_ensemble = 'Poissonian'
            )
            # A clause k, generates k*(k-1)/2 interactions
            maximum_yielded_couplers = num_stubs*(k-1)//2
            self.assertEqual(len(list(interactions)), maximum_yielded_couplers) 
            interactions = dimod.generators.satisfiability._kmcsat_interactions(num_variables, k, num_clauses,
                                                                                graph_ensemble = 'cutPoissonian'
            )
            self.assertEqual(len(list(interactions)), maximum_yielded_couplers) 
            # Configuration model is not enforceable:
            # Only 4 unique edges possible, but 8 requested (pathological)
            # Two clauses each with 2 variables, implies only 2 couplings come out:
        
            #with self.assertWarns(UserWarning):
            #    # Pathological model - cannot guarantee failure
            #    print(k)
            #    maximum_yielded_couplers = k*k
            #    interactions = dimod.generators.satisfiability._kmcsat_interactions(k, k, k+1,
            #                                                                        graph_ensemble = 'cutPoissonian',
            #                                                                        max_config_model_rejections = 2)
            
            self.assertLess(len(list(interactions)),maximum_yielded_couplers)
        
            # In BQM format, O(1) interactions overlap (sum) so there is no guarantee on edges
            bqm = dimod.generators.random_kmcsat(num_variables, k, num_clauses,
                                                            graph_ensemble = 'Poissonian')
            self.assertEqual(bqm.num_variables, num_variables)
            self.assertLessEqual(bqm.num_interactions, num_clauses*(k*(k-1))/2)
            ...
            bqm = dimod.generators.random_kmcsat(num_variables, k, num_clauses,
                                                 graph_ensemble = 'cutPoissonian',
                                                 max_config_model_rejections = 0)
            self.assertEqual(bqm.num_variables, num_variables)
            self.assertLessEqual(bqm.num_interactions, num_clauses*(k*(k-1))/2)

            vars_list = [[1,2,3]]
            signs_list = [[-1,1,-1]]
            dimod.generators.random_kmcsat(3,3,1,variables_list=vars_list)
            dimod.generators.random_kmcsat(3,3,1,signs_list=signs_list)
            dimod.generators.random_kmcsat(3,3,1,variables_list=vars_list,signs_list=signs_list)
        
            dimod.generators.random_kmcsat_cqm(variables_list_obj = vars_list,
                                               signs_list_obj = signs_list)
            dimod.generators.random_kmcsat_cqm(variables_list_cons = vars_list,
                                               signs_list_cons = signs_list)
            dimod.generators.random_kmcsat_cqm(variables_list_cons = vars_list,
                                               signs_list_cons = signs_list,
                                               constraint_form = 'linear')
            dimod.generators.random_kmcsat_cqm(variables_list_obj = vars_list,
                                               signs_list_obj = signs_list,
